...
const CourseDetail = ({ course, onBack, lectureProgress, setLectureProgress, setCourseProgress = ()=>{}, T, user, aiSettings, markStudyToday = ()=>{}, githubData = {} }) => {
                   ...
                   {/* Description + upload row */}
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12, marginBottom:10 }}>
                        ...
                        <div style={{ flexShrink:0 }}>
                          <label title="Upload .docx or .txt — Google Docs: File → Download → Word Document (.docx)"
                            style={{
                              display:"flex", alignItems:"center", gap:6, padding:"7px 13px",
                              borderRadius:8, border:"1px solid rgba(193,127,58,0.35)",
                              background:"rgba(193,127,58,0.08)", color:"#C17F3A",
                              fontSize:12, cursor:"pointer", fontFamily:"'JetBrains Mono',monospace",
                              whiteSpace:"nowrap", userSelect:"none",
                            }}>
                            {fileLoading === ps.id ? "⏳ Reading..." : "📎 Upload Doc"}
                            <input
                              type="file"
                              accept=".docx,.txt,.md"
                              style={{ display:"none" }}
                              onChange={e => handleFileUpload(ps.id, e.target.files?.[0])}
                            />
                          </label>
                      ...
                      <textarea
                        ...
                        placeholder={`Write your solution to ${ps.title} here…\n\nOr upload a Word doc above (Google Docs: File → Download → Word Document).`}
                        rows={10}
                        style={{
    ...
    <div>
      ...
      {viewMode === "grid" && (<>
        ...
        <div style={{ display:"flex", gap:10, marginBottom:20, alignItems:"center", flexWrap:"wrap" }}>
          ...
          {["All","Done","In Progress","Not Started"].map(f => (
...
const NoteTextarea = ({ value, onChange, inBg, txt }) => {
  ...
  return (
    <textarea
      ...
      onKeyDown={e => { if(e.key==="Enter" && !e.shiftKey){ e.preventDefault(); onChange(draft) }}}
